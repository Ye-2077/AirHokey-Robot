...
def safe_arccos(x):
    """ keep the input x in the domain of arccos function """
    if x < -1 or x > 1:
        # print(f"Input {x} is out of the domain of arccos function.")
        return None
    return np.arccos(x)
...
def inverse_kinematics(target_x,target_y,robot):
    ...
    alpha1 = safe_arccos((c2**2 - c1**2 - w**2) / (-2 * c1 * w))
    alpha2 = safe_arccos((-target_x + w) / c2)

    beta1 = safe_arccos((b1**2 - a1**2 - c1**2) / (-2 * a1 * c1))
    beta2 = safe_arccos((b2**2 - a2**2 - c2**2) / (-2 * a2 * c2))
    
    if None in (alpha1, alpha2, beta1, beta2):
